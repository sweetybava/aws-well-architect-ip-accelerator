...
# Function to catch a name collision between the deployer/bootstrap stack
# (the one that launched this EC2 instance, passed in via -a for auto-cleanup)
# and the CDK app's own target stack, which app.py always names
# "WA-IaC-Analyzer-<region>-GenAIStack". CloudFormation only allows one
# active stack per name in an account/region, so if the deployer stack was
# created with that exact name, `cdk deploy` will fail once the deployer
# stack reaches CREATE_IN_PROGRESS/COMPLETE with an error like:
#   "Stack:arn:...:stack/WA-IaC-Analyzer-<region>-GenAIStack/... is in
#    CREATE_IN_PROGRESS state and can not be updated."
# Catching it here saves ~15 minutes of build/push before that failure.
check_stack_name_collision() {
    local target_stack_name="WA-IaC-Analyzer-${REGION}-GenAIStack"
    if [ "$AUTO_CLEANUP" = true ] && [ "$DEPLOYMENT_STACK_NAME" = "$target_stack_name" ]; then
        echo "❌ Error: The deployer/bootstrap stack name ('$DEPLOYMENT_STACK_NAME')"
        echo "   is identical to the CDK application's own target stack name"
        echo "   ('$target_stack_name'), which app.py always generates from the"
        echo "   region. CloudFormation cannot have two active stacks with the"
        echo "   same name, so the real deployment will fail once it tries to"
        echo "   create/update that stack."
        echo ""
        echo "   Fix: delete this deployer stack and relaunch it under a"
        echo "   different name (e.g. 'WA-IaC-Analyzer-Deployer-${REGION}' or"
        echo "   'iac-analyzer-deployment-stack'), leaving"
        echo "   '$target_stack_name' free for the CDK app to create."
        exit 1
    fi
}
...
main() {
    ...
    # Fail fast if the deployer stack name collides with the CDK app's own
    # target stack name, before spending time on builds/pushes
    check_stack_name_collision
