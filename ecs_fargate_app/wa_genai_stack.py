...
from cdklabs.generative_ai_cdk_constructs import amazonaurora, bedrock, s3vectors
from constructs import Construct

# Vector store types supported by this stack. To add a future Bedrock-supported
# vector store (e.g. Pinecone, MongoDB Atlas, OpenSearch Managed Cluster), add
# its identifier here and extend `create_vector_store()` below - no other
# changes are required to switch between vector stores.
SUPPORTED_VECTOR_STORE_TYPES = ("OPENSEARCH_SERVERLESS", "AURORA", "S3_VECTORS")

# cdklabs.generative_ai_cdk_constructs' AmazonAuroraVectorStore hardcodes
# Aurora PostgreSQL engine version 15.5 with no exposed override, and AWS
# has since deprecated 15.5 (new AWS::RDS::DBCluster creation fails with
# "Cannot find version 15.5 for aurora-postgresql"). This Aspect reaches
# into the underlying L1 CfnDBCluster after construction and overrides just
# the EngineVersion property to a currently-supported 15.x minor version,
# without touching any of the VPC/IAM/pgvector-setup wiring the L3
# construct handles. Update AURORA_PG_ENGINE_VERSION if AWS deprecates this
# version too - see https://docs.aws.amazon.com/AmazonRDS/latest/AuroraPostgreSQLReleaseNotes/AuroraPostgreSQL.Updates.html
AURORA_PG_ENGINE_VERSION = "15.15"


@jsii.implements(cdk.IAspect)
class AuroraEngineVersionOverride:
    def visit(self, node):
        if isinstance(node, rds.CfnDBCluster):
            node.add_property_override("EngineVersion", AURORA_PG_ENGINE_VERSION)
...
class WAGenAIStack(Stack):
    ...
    def create_vector_store(
        ...
        """
        Returns the vector store construct (or None) to pass into
        bedrock.VectorKnowledgeBase(vector_store=...) based on the
        vector_store_type read from config.ini / the VectorStoreType
        CloudFormation parameter.

        Returning None for OPENSEARCH_SERVERLESS preserves the construct's
        default behavior (it provisions its own OpenSearch Serverless
        VectorCollection automatically), so no functional change occurs when
        vector_store_type is left at its default value.

        To add a future Bedrock-supported vector store, add a new elif branch
        here that constructs and returns the corresponding vector store
        construct from cdklabs.generative_ai_cdk_constructs (e.g. Pinecone,
        MongoDBAtlasVectorStore, or OpenSearchManagedClusterVectorStore), and
        add its identifier to SUPPORTED_VECTOR_STORE_TYPES.
        """
        if vector_store_type not in SUPPORTED_VECTOR_STORE_TYPES:
            raise ValueError(
                f"Unsupported vector_store_type '{vector_store_type}'. "
                f"Supported values are: {', '.join(SUPPORTED_VECTOR_STORE_TYPES)}"
            )

        if vector_store_type == "OPENSEARCH_SERVERLESS":
            # Let bedrock.VectorKnowledgeBase provision its own OpenSearch
            # Serverless VectorCollection (existing/default behavior).
            return None

        if vector_store_type == "AURORA":
            # Provisions its own VPC + Aurora Serverless v2 PostgreSQL
            # cluster with pgvector, configured for use as a Bedrock
            # Knowledge Base vector store. Note: this construct takes
            # significantly longer to deploy/destroy (~20-25 minutes)
            # than OpenSearch Serverless.
            aurora_store = amazonaurora.AmazonAuroraVectorStore(
                self,
                "WAFR-AuroraVectorStore",
                embeddings_model_vector_dimension=embeddings_model.vector_dimensions,
            )
            # Fix hardcoded/deprecated engine version - see
            # AuroraEngineVersionOverride definition above.
            cdk.Aspects.of(aurora_store).add(AuroraEngineVersionOverride())
            return aurora_store

        if vector_store_type == "S3_VECTORS":
            # Provisions an Amazon S3 Vectors bucket + vector index and
            # passes the index directly as the knowledge base's vector
            # store. S3 Vectors is the lowest-cost option since there is
            # no always-on compute/cluster to run - vectors are stored
            # and queried directly from S3.
            ...
            return s3vectors.VectorIndex(
                self,
                "WAFR-S3VectorIndex",
                vector_bucket=vector_bucket,
                dimension=embeddings_model.vector_dimensions,
                # Keep the raw chunk text out of the filterable metadata
                # (recommended by AWS for the Bedrock-managed text field)
                # while still allowing it to be retrieved with results.
                non_filterable_metadata_keys=["AMAZON_BEDROCK_TEXT"],
            )

        # Unreachable given the check above, kept for safety.
        raise ValueError(f"Unhandled vector_store_type '{vector_store_type}'")
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwarg) -> None:
        ...
        vector_store_type = (
            config["settings"]
            .get("vector_store_type", fallback="OPENSEARCH_SERVERLESS")
            .strip()
            .upper()
        )
        ...
        # Build the vector store selected via config.ini's vector_store_type
        # (or the VectorStoreType CloudFormation parameter). This is what
        # allows switching between Bedrock-supported vector stores by
        # changing a single configuration value at deployment time, without
        # maintaining separate branches/versions of the application.
        embeddings_model = bedrock.BedrockFoundationModel.TITAN_EMBED_TEXT_V2_1024
        vector_store = self.create_vector_store(vector_store_type, embeddings_model)

        # Creates Bedrock KB using the generative_ai_cdk_constructs
        # NOTE: bedrock.KnowledgeBase was renamed to bedrock.VectorKnowledgeBase
        # in newer releases of cdklabs.generative_ai_cdk_constructs (required
        # for S3 Vectors support). See requirements.txt.
        ...
        # Output which vector store type backs the Bedrock knowledge base
        cdk.CfnOutput(
            self,
            "VectorStoreType",
            value=vector_store_type,
            description="Vector store type backing the Bedrock knowledge base",
        )
