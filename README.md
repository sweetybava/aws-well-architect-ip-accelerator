...
- **Bedrock Knowledge Base Vector Store** (`VectorStoreType`)
  - `OPENSEARCH_SERVERLESS` (Default): Provisions an Amazon OpenSearch Serverless collection as the Knowledge Base vector store. Fastest option to deploy/destroy.
  - `AURORA`: Provisions an Amazon Aurora PostgreSQL Serverless v2 cluster with the pgvector extension as the Knowledge Base vector store, including its own VPC. Deploying and destroying this option takes significantly longer (~20-25 minutes).
  - `S3_VECTORS`: Provisions an Amazon S3 Vectors bucket and vector index as the Knowledge Base vector store. Lowest-cost option since there's no always-on compute/cluster - vectors are stored and queried directly from S3. Fast to deploy/destroy, similar to OpenSearch Serverless.
  - Changing this parameter switches the vector store used by the Bedrock Knowledge Base at deployment time - no code changes are required. Additional Bedrock-supported vector stores can be added in the future by extending the allowed values and the corresponding logic in `wa_genai_stack.py`.
...
### Bedrock Knowledge Base Vector Store Selection

By default, this project provisions an **Amazon OpenSearch Serverless** collection as the vector store for the Bedrock Knowledge Base. You can switch to a different Bedrock-supported vector store by changing the `vector_store_type` parameter in the config.ini file:
```ini
[settings]
...
Supported values:
- `OPENSEARCH_SERVERLESS` (Default): Amazon OpenSearch Serverless. Fastest to deploy and destroy.
- `AURORA`: Amazon Aurora PostgreSQL Serverless v2 with pgvector. Provisions its own VPC and Aurora cluster; deploying and destroying this option takes significantly longer (~20-25 minutes).
- `S3_VECTORS`: Amazon S3 Vectors. Lowest-cost option, no always-on compute/cluster - vectors are stored and queried directly from an S3 vector bucket. Fast to deploy/destroy, similar to OpenSearch Serverless.

> **Note:** Switching this value changes which vector store gets deployed the next time you run `cdk deploy` / `deploy-wa-analyzer.sh` - no code changes or separate branches are needed. Support for additional Bedrock-supported vector stores (e.g. Pinecone, MongoDB Atlas, OpenSearch Managed Cluster) can be added by extending `create_vector_store()` in `ecs_fargate_app/wa_genai_stack.py`.
